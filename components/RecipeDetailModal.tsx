    ...
    <Modal isOpen={true} onClose={onClose}>
      <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl mx-auto max-h-[90vh] flex flex-col">
        ...
        <div className="p-5 overflow-y-auto flex-grow">
          <div className="mb-6 space-y-3">
            ...
            {recipe.mainIngredients && recipe.mainIngredients.length > 0 && (
              <div className="flex items-start text-lg text-gray-700">
                <TagIcon className="w-6 h-6 mr-3 mt-1 text-sky-500 flex-shrink-0" />
                <div>
          ...
          >
            {isFavorite ? <FilledStarIcon className="w-6 h-6 mr-2" /> : <StarIcon className="w-6 h-6 mr-2" />}
          ...
          <DetailSection title="作り方" icon={<SparklesIcon className="w-6 h-6 text-sky-500" />} className="border-t border-gray-200 pt-6">
            ...
            <p className="mt-4 text-sm text-gray-500 flex items-center">
              <CheckCircleIcon className="w-5 h-5 text-sky-500 mr-2" />
            ...
            <DetailSection title="栄養詳細 (推定)" icon={<ChartPieIcon className="w-6 h-6 text-teal-500" />} className="border-t border-gray-200 pt-6">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                ...
                {recipe.nutrition.fat && (
                  <div className="bg-sky-50 p-3 rounded-lg shadow-sm border border-sky-100 text-center">
                    <p className="font-semibold text-sky-700 mb-1">脂質</p>
                    <p className="text-gray-700 text-lg">{recipe.nutrition.fat}</p>
                  </div>
                )}
                {recipe.nutrition.carbs && (
                  <div className="bg-teal-50 p-3 rounded-lg shadow-sm border border-teal-100 text-center">
                    <p className="font-semibold text-teal-700 mb-1">炭水化物</p>
          ...
          {recipe.tips && (
            <DetailSection title="料理のコツ・アレンジ案" icon={<SparklesIcon className="w-6 h-6 text-sky-400" />} className="border-t border-gray-200 pt-6">
