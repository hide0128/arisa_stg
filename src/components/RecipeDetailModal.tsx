    ...
    <Modal isOpen={true} onClose={onClose}>
      <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl mx-auto max-h-[90vh] flex flex-col">
        <div className="p-5 border-b border-gray-200 flex justify-between items-start sticky top-0 bg-white z-10">
          <div>
            <h2 className="text-3xl font-bold text-orange-600">{recipe.name}</h2>
        ...
        <div className="p-5 overflow-y-auto flex-grow">
          ...
          <DetailSection title="材料リスト" icon={<UsersIcon className="w-6 h-6 text-blue-500" />} className="border-t border-gray-200 pt-6">
            <ul className="space-y-1.5 list-none text-gray-700">
              {recipe.ingredients.map((item, index) => (
                <li key={index} className="ml-4 p-2 bg-orange-50/50 rounded-md border border-orange-100 flex justify-between items-center">
                  <span className="text-gray-800">{item.name}</span> <span className="font-medium text-orange-600">{item.quantity}</span>
                </li>
              ))}
            </ul>
          </DetailSection>
            ...
            <ol className="space-y-3 text-gray-700">
              {recipe.instructions.map((step, index) => (
                <li key={index} className="flex items-start">
                  <span className="bg-orange-500 text-white rounded-full h-7 w-7 text-sm flex items-center justify-center mr-3 mt-0.5 flex-shrink-0 font-semibold">{index + 1}</span>
          ...
          {recipe.tips && (
            <DetailSection title="料理のコツ・アレンジ案" icon={<SparklesIcon className="w-6 h-6 text-yellow-500" />} className="border-t border-gray-200 pt-6">
              <p className="text-gray-700 whitespace-pre-line bg-amber-50 p-4 rounded-md border border-amber-200 leading-relaxed">{recipe.tips}</p>
            </DetailSection>
          )}
        </div>
